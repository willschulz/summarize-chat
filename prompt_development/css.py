...
def parse_args():
    ...
    parser.add_argument("--chat-path", type=Path, default='')
...
class ChatAgentTUI():
    def __init__(self, args):
        ...
        with open(Path(self.args.chat_path)) as csvfile:
    ...
    def save_conversation(self, local_log: bool=True):
        ...
        # Create local log
        if local_log:
            filename = os.path.basename(self.args.chat_path)
            savefile = Path(
                "logs/{}_chatbot_log_{}.json".format(
                    filename[:-4], dt.now().strftime('%Y%m%d-%H%M%S')))
            savefile.write_text(json.dumps(history))
            savefile = Path(
                "logs/{}_chatbot_token-usage_{}.json".format(
                    filename[:-4], dt.now().strftime('%Y%m%d-%H%M%S')))
